...
import { username, password, apiKey, bearerToken } from "./api_keys.js";
...
app.get("/basicAuth", async (req, res) => {
  ...
  */
    try {
        const response = await axios.get(
            "https://secrets-api.appbrewery.com/all",
            {
                auth: {
        ...
        const result = response.data;
        const randomResult = Math.floor(Math.random() * response.data.length);
        console.log(result[randomResult], randomResult);
        res.render("index.ejs", { content: result[randomResult].secret });
    } catch (error) {
        console.log(`Error: ${error.message}`);
        res.render("index.ejs", { content: error.message });
    }
